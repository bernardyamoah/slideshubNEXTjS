export default function Footer() {
  return (
    <>
      <div className="absolute inset-x-0 top-[calc(5%)] -z-[5]  overflow-hidden blur-3xl sm:-top-[calc(20%)]">
        <div
          className="relative left-[calc(50%+3rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 bg-gradient-to-tr from-emerald-300 to-emerald-600 opacity-30 sm:left-[calc(50%+36rem)] sm:w-[72.1875rem]"
          style={{
            clipPath:
              "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
          }}
        ></div>
      </div>
      <footer className="fixed hidden overflow-hidden  inset-x-0 bottom-0 w-full p-4 border-t bg-zinc-800 z-[50] ">
        <div className="flex justify-between text-sm text-center text-gray-100">
          <p>Slideshub</p>
          <p>
            &copy; {new Date().getFullYear()} Slideshub. All rights reserved.
          </p>
        </div>
      </footer>
    </>
  );
}
